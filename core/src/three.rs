...
/// Searches for the first occurrence of a three-byte needle in a byte slice haystack
///
/// ## Example
///
/// ```
...
/// assert_eq!(search_three(haystack, *b"ell"), Some(0x01));
/// assert_eq!(search_three(haystack, *b"rld"), Some(0x08));
/// assert_eq!(search_three(haystack, *b"zzz"), None);
/// ```
#[inline(always)]
#[cfg(target_pointer_width = "64")]
pub fn search_three(haystack: &[u8], needle: [u8; 0x03]) -> Option<usize> {
    #[cfg(target_arch = "x86_64")]
    match get_cpu_feature() {
        ISA::SWAR => search_three_swar64(haystack, needle),
        ISA::SSE2 => unsafe { search_three_sse2(haystack, needle) },
        ISA::SSSE3 => unsafe { search_three_ssse3(haystack, needle) },
        ISA::SSE4_2 => unsafe { search_three_sse42(haystack, needle) },
        ISA::AVX2 => unsafe { search_three_avx2(haystack, needle) },
        ISA::AVX512BW => unsafe { search_three_avx512(haystack, needle) },
        _ => unreachable!(),
    }
    ...
    #[cfg(target_arch = "aarch64")]
    {
        ...
        return search_three_swar64(haystack, needle);

        unsafe { search_three_neon(haystack, needle) }
    }

    #[cfg(not(any(target_arch = "x86_64", target_arch = "aarch64")))]
    {
        search_three_swar64(haystack, needle)
    }
}
...
pub fn search_three(haystack: &[u8], needle: [u8; 0x03]) -> Option<usize> {
    #[cfg(target_arch = "wasm32")]
    {
        ...
        return unsafe { search_three_simd128(haystack, needle) };

        search_three_swar32(haystack, needle)
    }

    #[cfg(target_arch = "arm")]
    {
        #[cfg(target_feature = "neon")]
        return unsafe { search_three_neon(haystack, needle) };
    ...
    #[cfg(target_arch = "x86")]
    {
        ...
        return unsafe { search_three_sse42(haystack, needle) };

        #[cfg(target_feature = "ssse3")]
        return unsafe { search_three_ssse3(haystack, needle) };

        #[cfg(target_feature = "sse2")]
        return unsafe { search_three_sse2(haystack, needle) };
...
#[inline(always)]
#[cfg(any(target_pointer_width = "64", test))]
fn search_three_swar64(haystack: &[u8], needle: [u8; 0x03]) -> Option<usize> {
...
#[inline(always)]
#[cfg(any(target_pointer_width = "32", test))]
fn search_three_swar32(haystack: &[u8], needle: [u8; 0x03]) -> Option<usize> {
...
unsafe fn search_three_sse2(haystack: &[u8], needle: [u8; 0x03]) -> Option<usize> {
    let v_needle_a = _mm_set1_epi8(needle[0x00] as i8);
    let v_needle_b = _mm_set1_epi8(needle[0x01] as i8);
    let v_needle_c = _mm_set1_epi8(needle[0x02] as i8);
    ...
    while i + 0x12 <= len {
        ...
        let v_c = _mm_loadu_si128(ptr.add(i + 0x02) as *const __m128i);
...
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
unsafe fn search_three_ssse3(haystack: &[u8], needle: [u8; 0x03]) -> Option<usize> {
    let v_needle_a = _mm_set1_epi8(needle[0x00] as i8);
    let v_needle_b = _mm_set1_epi8(needle[0x01] as i8);
    let v_needle_c = _mm_set1_epi8(needle[0x02] as i8);
    ...
    while i + 0x50 <= len {
        ...
        let v1_c = _mm_alignr_epi8(v2, v1, 0x02);
        let v2_b = _mm_alignr_epi8(v3, v2, 0x01);
        let v2_c = _mm_alignr_epi8(v3, v2, 0x02);
        let v3_b = _mm_alignr_epi8(v4, v3, 0x01);
        let v3_c = _mm_alignr_epi8(v4, v3, 0x02);
        let v4_b = _mm_alignr_epi8(v5, v4, 0x01);
        let v4_c = _mm_alignr_epi8(v5, v4, 0x02);
...
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
unsafe fn search_three_sse42(haystack: &[u8], needle: [u8; 0x03]) -> Option<usize> {
    let v_needle_a = _mm_set1_epi8(needle[0x00] as i8);
    let v_needle_b = _mm_set1_epi8(needle[0x01] as i8);
    let v_needle_c = _mm_set1_epi8(needle[0x02] as i8);
...
unsafe fn search_three_avx2(haystack: &[u8], needle: [u8; 0x03]) -> Option<usize> {
    let v_needle_a = _mm256_set1_epi8(needle[0x00] as i8);
    let v_needle_b = _mm256_set1_epi8(needle[0x01] as i8);
    let v_needle_c = _mm256_set1_epi8(needle[0x02] as i8);
    ...
    while i + 0x42 <= len {
        ...
        let v1_c = _mm256_loadu_si256(ptr.add(i + 0x02) as *const __m256i);
        let v2_a = _mm256_loadu_si256(ptr.add(i + 0x20) as *const __m256i);
        let v2_b = _mm256_loadu_si256(ptr.add(i + 0x21) as *const __m256i);
        let v2_c = _mm256_loadu_si256(ptr.add(i + 0x22) as *const __m256i);
    ...
    if i + 0x22 <= len {
        ...
        let v_c = _mm256_loadu_si256(ptr.add(i + 0x02) as *const __m256i);
...
unsafe fn search_three_avx512(haystack: &[u8], needle: [u8; 0x03]) -> Option<usize> {
    let v_needle_a = _mm512_set1_epi8(needle[0x00] as i8);
    let v_needle_b = _mm512_set1_epi8(needle[0x01] as i8);
    let v_needle_c = _mm512_set1_epi8(needle[0x02] as i8);
    ...
    while i + 0x82 <= len {
        ...
        let v1_c = _mm512_loadu_si512(ptr.add(i + 0x02) as *const _);
        let v2_a = _mm512_loadu_si512(ptr.add(i + 0x40) as *const _);
        let v2_b = _mm512_loadu_si512(ptr.add(i + 0x41) as *const _);
        let v2_c = _mm512_loadu_si512(ptr.add(i + 0x42) as *const _);

        let eq1 = _mm512_cmpeq_epi8_mask(v1_a, v_needle_a)
            & _mm512_cmpeq_epi8_mask(v1_b, v_needle_b)
            & _mm512_cmpeq_epi8_mask(v1_c, v_needle_c);
        let eq2 = _mm512_cmpeq_epi8_mask(v2_a, v_needle_a)
    ...
    if i + 0x42 <= len {
        ...
        let v_c = _mm512_loadu_si512(ptr.add(i + 0x02) as *const _);

        let eq = _mm512_cmpeq_epi8_mask(v_a, v_needle_a)
...
#[inline(always)]
#[cfg(target_arch = "aarch64")]
unsafe fn search_three_neon(haystack: &[u8], needle: [u8; 0x03]) -> Option<usize> {
    #[inline(always)]
    ...
    let v_needle_c = vdupq_n_u8(needle[0x02]);
    ...
    while i + 0x50 <= len {
        ...
        let v1_c = vextq_u8(v1, v2, 0x02);
        let v2_b = vextq_u8(v2, v3, 0x01);
        let v2_c = vextq_u8(v2, v3, 0x02);
        let v3_b = vextq_u8(v3, v4, 0x01);
        let v3_c = vextq_u8(v3, v4, 0x02);
        let v4_b = vextq_u8(v4, v5, 0x01);
        let v4_c = vextq_u8(v4, v5, 0x02);

        let eq1 = vandq_u8(
            vandq_u8(vceqq_u8(v1, v_needle_a), vceqq_u8(v1_b, v_needle_b)),
            vceqq_u8(v1_c, v_needle_c),
        );
        let eq2 = vandq_u8(
            vandq_u8(vceqq_u8(v2, v_needle_a), vceqq_u8(v2_b, v_needle_b)),
            vceqq_u8(v2_c, v_needle_c),
        );
        let eq3 = vandq_u8(
            vandq_u8(vceqq_u8(v3, v_needle_a), vceqq_u8(v3_b, v_needle_b)),
            vceqq_u8(v3_c, v_needle_c),
        );
        let eq4 = vandq_u8(
            vandq_u8(vceqq_u8(v4, v_needle_a), vceqq_u8(v4_b, v_needle_b)),
    ...
    while i + 0x12 <= len {
        ...
        let v_c = vld1q_u8(ptr.add(i + 0x02));
...
unsafe fn search_three_simd128(haystack: &[u8], needle: [u8; 0x03]) -> Option<usize> {
    let v_needle_a = u8x16_splat(needle[0x00]);
    let v_needle_b = u8x16_splat(needle[0x01]);
    let v_needle_c = u8x16_splat(needle[0x02]);
    ...
    while i + 0x50 <= len {
        ...
        let eq1 = v128_and(
            v128_and(u8x16_eq(v1, v_needle_a), u8x16_eq(v1_b, v_needle_b)),
            u8x16_eq(v1_c, v_needle_c),
        );
        let eq2 = v128_and(
            v128_and(u8x16_eq(v2, v_needle_a), u8x16_eq(v2_b, v_needle_b)),
            u8x16_eq(v2_c, v_needle_c),
        );
        let eq3 = v128_and(
            v128_and(u8x16_eq(v3, v_needle_a), u8x16_eq(v3_b, v_needle_b)),
            u8x16_eq(v3_c, v_needle_c),
        );
        let eq4 = v128_and(
            v128_and(u8x16_eq(v4, v_needle_a), u8x16_eq(v4_b, v_needle_b)),
    ...
    while i + 0x12 <= len {
        ...
        let v_c = v128_load(ptr.add(i + 0x02) as *const v128);
...
mod tests {
    ...
    fn run_standard_suite(search_fn: impl Fn(&[u8], [u8; 0x03]) -> Option<usize>) {
        assert_eq!(search_fn(b"", *b"abc"), None);

        assert_eq!(search_fn(b"a", *b"abc"), None);
        assert_eq!(search_fn(b"ab", *b"abc"), None);
        assert_eq!(search_fn(b"ba", *b"abc"), None);

        assert_eq!(search_fn(b"abc", *b"abc"), Some(0x00));
        assert_eq!(search_fn(b"abd", *b"abc"), None);
        assert_eq!(search_fn(b"bac", *b"abc"), None);

        let haystack = b"the quick brown fox jumps over the lazy dog";
        assert_eq!(search_fn(haystack, *b"ZZZ"), None);
        assert_eq!(search_fn(haystack, *b"!!!"), None);
        assert_eq!(search_fn(haystack, *b"the"), Some(0x00));
        assert_eq!(search_fn(haystack, *b"qui"), Some(0x04));
        assert_eq!(search_fn(haystack, *b"bro"), Some(0x0A));
        assert_eq!(search_fn(haystack, *b"fox"), Some(0x10));
        assert_eq!(search_fn(haystack, *b"dog"), Some(0x28));

        for len in 0x03..0x0A {
            let mut h = vec![b'x'; len];
            for pos in 0x00..len - 0x02 {
                h[pos] = b'a';
                h[pos + 0x01] = b'b';
                h[pos + 0x02] = b'c';
                assert_eq!(
        ...
        h8[0x06] = b'B';
        h8[0x07] = b'C';
        assert_eq!(search_fn(&h8, *b"ABC"), Some(0x05));

        h9[0x06] = b'A';
        h9[0x07] = b'B';
        h9[0x08] = b'C';
        assert_eq!(search_fn(&h9, *b"ABC"), Some(0x06));

        h16[0x0D] = b'C';
        h16[0x0E] = b'D';
        h16[0x0F] = b'E';
        assert_eq!(search_fn(&h16, *b"CDE"), Some(0x0D));

        h17[0x0E] = b'C';
        h17[0x0F] = b'D';
        h17[0x10] = b'E';
        assert_eq!(search_fn(&h17, *b"CDE"), Some(0x0E));

        h18[0x0F] = b'C';
        h18[0x10] = b'D';
        h18[0x11] = b'E';
        assert_eq!(search_fn(&h18, *b"CDE"), Some(0x0F));

        h24[0x15] = b'E';
        h24[0x16] = b'F';
        h24[0x17] = b'G';
        assert_eq!(search_fn(&h24, *b"EFG"), Some(0x15));

        h25[0x16] = b'E';
        h25[0x17] = b'F';
        h25[0x18] = b'G';
        assert_eq!(search_fn(&h25, *b"EFG"), Some(0x16));

        h32[0x1D] = b'G';
        h32[0x1E] = b'H';
        h32[0x1F] = b'I';
        assert_eq!(search_fn(&h32, *b"GHI"), Some(0x1D));

        h33[0x1E] = b'G';
        h33[0x1F] = b'H';
        h33[0x20] = b'I';
        assert_eq!(search_fn(&h33, *b"GHI"), Some(0x1E));

        h34[0x1F] = b'G';
        h34[0x20] = b'H';
        h34[0x21] = b'I';
        assert_eq!(search_fn(&h34, *b"GHI"), Some(0x1F));

        h35[0x20] = b'G';
        h35[0x21] = b'H';
        h35[0x22] = b'I';
        assert_eq!(search_fn(&h35, *b"GHI"), Some(0x20));

        let cross_positions = [
            0x02, 0x03, 0x04, 0x06, 0x07, 0x08, 0x0E, 0x0F, 0x10, 0x11, 0x1E, 0x1F, 0x20, 0x21,
            0x26, 0x27, 0x28, 0x3E, 0x3F, 0x40,
        ];
        let mut cross_buf = vec![b'-'; 0x80];
        for &pos in &cross_positions {
            cross_buf[pos] = b'X';
            cross_buf[pos + 0x01] = b'Y';
            cross_buf[pos + 0x02] = b'Z';
            assert_eq!(
                search_fn(&cross_buf, *b"XYZ"),
                Some(pos),
            ...
            cross_buf[pos + 0x02] = b'-';
        }

        let mut haystack = vec![b'-'; 0x200];
        for i in 0x00..haystack.len() - 0x02 {
            haystack[i] = b'A';
            haystack[i + 0x01] = b'B';
            haystack[i + 0x02] = b'C';
            assert_eq!(
                search_fn(&haystack, *b"ABC"),
                Some(i),
        ...
        let mut haystack_partial = vec![b'A'; 0x100];
        haystack_partial[0x7A] = b'B';
        assert_eq!(search_fn(&haystack_partial, *b"ABC"), None);

        haystack_partial[0x7B] = b'C';
        assert_eq!(search_fn(&haystack_partial, *b"ABC"), Some(0x79));

        let mut haystack_middle = vec![b'A'; 0x100];
        haystack_middle[0x50] = b'X';
        haystack_middle[0x51] = b'C';
        assert_eq!(search_fn(&haystack_middle, *b"ABC"), None);

        let mut haystack_suffix = vec![b'X'; 0x100];
        haystack_suffix[0x40] = b'A';
        haystack_suffix[0x41] = b'B';
        haystack_suffix[0x42] = b'X';
        assert_eq!(search_fn(&haystack_suffix, *b"ABC"), None);

        haystack_suffix[0x42] = b'C';
        assert_eq!(search_fn(&haystack_suffix, *b"ABC"), Some(0x40));

        assert_eq!(search_fn(b"aaaaaa", *b"aaa"), Some(0x00));
        assert_eq!(search_fn(b"baaaaa", *b"aaa"), Some(0x01));
        assert_eq!(search_fn(b"bbaaaa", *b"aaa"), Some(0x02));
        assert_eq!(search_fn(b"aababc", *b"abc"), Some(0x03));
        assert_eq!(search_fn(b"ababc", *b"abc"), Some(0x02));
        assert_eq!(search_fn(b"abcabc", *b"abc"), Some(0x00));
        assert_eq!(search_fn(b"zabcabc", *b"abc"), Some(0x01));
        ...
        assert_eq!(search_fn(&haystack_high, [0x7E, 0x7F, 0x80]), None);
        assert_eq!(search_fn(&haystack_high, [0xFD, 0xFE, 0xFF]), Some(0x3D));

        let mut haystack_null = vec![0xFF; 0x50];
        haystack_null[0x2A] = 0x00;
        haystack_null[0x2B] = 0x00;
        haystack_null[0x2C] = 0x00;
        assert_eq!(search_fn(&haystack_null, [0x00, 0x00, 0x00]), Some(0x2A));

        assert_eq!(
            search_fn(b"\x01\x01\x01\x01\x01\x01\x01\x01\x01", [0x01, 0x01, 0x01]),
            Some(0x00)
        );
        assert_eq!(
            search_fn(b"\x80\x80\x80\x80\x80\x80\x80\x80\x80", [0x80, 0x80, 0x80]),
            Some(0x00)
        );
        assert_eq!(
            search_fn(b"\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF", [0xFF, 0xFF, 0xFF]),
            Some(0x00)
        );
        assert_eq!(
            search_fn(b"\x00\x00\x00\x00\x00\x00\x00\x00\x00", [0x00, 0x00, 0x00]),
            Some(0x00)
        );
        ...
        for offset in 0x01..0x08 {
            ...
            h[0x1A] = b'Y';
            h[0x1B] = b'Z';
            assert_eq!(search_fn(&h, *b"XYZ"), Some(0x19));

            let end_idx = h.len() - 0x03;
            h[end_idx] = b'J';
            h[end_idx + 0x01] = b'K';
            h[end_idx + 0x02] = b'L';
            assert_eq!(search_fn(&h, *b"JKL"), Some(end_idx));
        }

        let tail_lengths = [
            0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13,
            0x1F, 0x20, 0x21, 0x22, 0x23, 0x2F, 0x30, 0x31, 0x32, 0x3F, 0x40, 0x41, 0x42, 0x7F,
            0x80, 0x81, 0x82, 0xFF, 0x100, 0x101, 0x102,
        ];
        ...
        haystack_multi[0x12] = b'O';
        haystack_multi[0x50] = b'M';
        haystack_multi[0x51] = b'N';
        haystack_multi[0x52] = b'O';
        assert_eq!(search_fn(&haystack_multi, *b"MNO"), Some(0x10));

        let mut huge_haystack = vec![b'x'; 0x64 * 0x400];
        assert_eq!(search_fn(&huge_haystack, *b"XYZ"), None);

        let last_pos = 0x64 * 0x400 - 0x03;
        huge_haystack[last_pos] = b'X';
        huge_haystack[last_pos + 0x01] = b'Y';
        huge_haystack[last_pos + 0x02] = b'Z';
        assert_eq!(search_fn(&huge_haystack, *b"XYZ"), Some(last_pos));
    }

    #[test]
    fn test_public_api() {
        run_standard_suite(search_three);
    }

    #[test]
    #[cfg(any(target_pointer_width = "64", test))]
    fn test_swar64_directly() {
        run_standard_suite(search_three_swar64);
    }

    #[test]
    #[cfg(any(target_pointer_width = "32", test))]
    fn test_swar32_directly() {
        run_standard_suite(search_three_swar32);
    }
    ...
    fn test_sse2_directly() {
        if std::is_x86_feature_detected!("sse2") {
            run_standard_suite(|h, n| unsafe { search_three_sse2(h, n) });
        }
    }
    ...
    fn test_ssse3_directly() {
        if std::is_x86_feature_detected!("ssse3") {
            run_standard_suite(|h, n| unsafe { search_three_ssse3(h, n) });
        }
    }
    ...
    fn test_sse42_directly() {
        if std::is_x86_feature_detected!("sse4.2") {
            run_standard_suite(|h, n| unsafe { search_three_sse42(h, n) });
        }
    }
    ...
    fn test_avx2_directly() {
        if std::is_x86_feature_detected!("avx2") {
            run_standard_suite(|h, n| unsafe { search_three_avx2(h, n) });
        }
    }
    ...
    fn test_avx512_directly() {
        if std::is_x86_feature_detected!("avx512bw") {
            run_standard_suite(|h, n| unsafe { search_three_avx512(h, n) });
        }
    }
    ...
    fn test_neon_aarch64_directly() {
        run_standard_suite(|h, n| unsafe { search_three_neon(h, n) });
    }
    ...
    fn test_simd128_directly() {
        run_standard_suite(|h, n| unsafe { search_three_simd128(h, n) });
    }
}
