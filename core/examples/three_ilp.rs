//! ILP (Instruction-Level Parallelism) and Hardware Profiling Harness for Ashwa (search_three)
//!
...
/// based on the compiled hardware vector ISA extension for search_three.
fn estimate_instructions_per_search(size: usize) -> u64 {
    #[cfg(all(target_arch = "x86_64", target_feature = "avx512bw"))]
    {
        // AVX-512BW loop unrolls 128 bytes per iteration:
        // 6 x vmovdqu8 + 3 x vpcmpb + 2 x vand + 1 x vor + 1 x branch/loop = 13 instructions per 128-byte block
        let num_128b_blocks = (size / 128) as u64;
        num_128b_blocks * 13 + 4
    }

    #[cfg(all(target_arch = "x86_64", target_feature = "avx2", not(target_feature = "avx512bw")))]
    {
        // AVX2 loop unrolls 64 bytes per iteration:
        // 6 x vmovdqu + 3 x vpcmpeqb + 3 x vpand + 1 x vpor + 1 x vpmovmskb + 1 x loop = 15 instructions per 64-byte block
    ...
    #[cfg(all(target_arch = "x86_64", not(target_feature = "avx2"), not(target_feature = "avx512bw")))]
    {
        ...
        // 5 x vmovdqu + 8 x alignr + 4 x vpcmpeqb + 8 x vpand + 3 x vpor + 1 x vpmovmskb + 1 x loop = 30 instructions per 64-byte block
        let num_64b_blocks = (size / 64) as u64;
        num_64b_blocks * 30 + 4
    }
    ...
    #[cfg(target_arch = "aarch64")]
    {
        ...
        // 5 x vld1q_u8 + 8 x vextq_u8 + 12 x vceqq_u8 + 8 x vandq_u8 + 3 x vorrq_u8 + 2 x fmov + 1 x orr + 1 x branch/loop = 40 instructions per 64-byte block
        let num_64b_blocks = (size / 64) as u64;
        num_64b_blocks * 40 + 4
    }
...
fn run_tier(tier: &TierConfig, needle: [u8; 3]) {
    let size = tier.size;
    ...
    // Warmup
    for _ in 0..10 {
        black_box(search_three(black_box(slice), black_box(needle)));
    }
...
fn main() {
    ...
    let needle = [0x0Au8, 0x0Bu8, 0x0Cu8];
